...
int deathFrameOrder[] = {12, 12, 12, 13, 13, 13, 14, 14, 14, 15, 15,
...
int preMovePlayer(Player* player) {
  ...
  if (current > 3) {
    pickupPowerUp((player->y + 16 - YOFFSET) >> 5,
                  (player->x + 16 - XOFFSET) >> 5);
    if (current & BOMB) {
...
Bomb* playerPlaceBomb(Player* player) {
  if (player->nBombsInUse < player->nBombs) {
    Bomb* b = placeBomb(player->clientNum, (16 + player->y - YOFFSET) >> 5,
                        (16 + player->x - XOFFSET) >> 5);
    if (b == NULL)
      return NULL;
...
int drawPlayer(Player* player) {
  if (player->state == PLAYER_NORMAL) {
    if (player->dir == UP) {
      DDraw_Draw_Surface(player->textures[0 + frameOrder[player->frame]],
                         player->x, player->y - 32, 32, 64, surface, 1);
    } else if (player->dir == DOWN) {
      DDraw_Draw_Surface(player->textures[3 + frameOrder[player->frame]],
                         player->x, player->y - 32, 32, 64, surface, 1);
    } else if (player->dir == LEFT) {
      DDraw_Draw_Surface(player->textures[6 + frameOrder[player->frame]],
                         player->x, player->y - 32, 32, 64, surface, 1);
    } else if (player->dir == RIGHT) {
      DDraw_Draw_Surface(player->textures[9 + frameOrder[player->frame]],
                         player->x, player->y - 32, 32, 64, surface, 1);
    }
    ...
    DDraw_DrawSized_Surface(
        player->textures[deathFrameOrder[player->frame]],
        player->x - player->frame, player->y - 32 - player->frame, 32, 64,
        32 + (player->frame << 1), 64 + (player->frame << 1), surface, 1);
    player->frame++;
