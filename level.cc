...
int brickBreakingFrameOrder[BRICK_BREAKING_NUM_FRAMES]={4,4,4,5,5,5,6,6,6,7,7,7,8,8,8,9,9,9,10,10,10};
...
Bomb * placeBomb(int playerNum,int i,int j)
{
	...
	while(index<TOTAL_BOMBS)
	{
		...
		{
			int temp;
			initBomb(&bombs[index],(XOFFSET) + (j<<5),(YOFFSET)+(i<<5),BOMB_NORMAL,1,NULL);
...
int createBasicLevel(int nbombs,int nflames, int nkick, int ndet, int nspeed)
{
	...
	int floori[12]={0,0,1,
					0,0,1,
					GRID_HEIGHT-1,GRID_HEIGHT-1,GRID_HEIGHT-2,
					GRID_HEIGHT-1,GRID_HEIGHT-1,GRID_HEIGHT-2};
	int floorj[12]={0,1,0,
					GRID_WIDTH-1,GRID_WIDTH-2,GRID_WIDTH-1,
					GRID_WIDTH-1,GRID_WIDTH-2,GRID_WIDTH-1,
					0,1,0};
	i=0;
	while(i<GRID_HEIGHT)
...
int levelKillPlayer(Bomb * bomb,int i, int j)
{
	...
	{
		Player * player = (Player *)finger->value;
		if((player->x+16-XOFFSET)>>5==j && (player->y+16-YOFFSET)>>5==i)
...
/*
	...
	returns BURN_STOP if the flame is suppsed to burn this square and stop after
*/
...
#ifdef DEBUG
	char labels[][25]={"","Burn Through","Burn Stop","Burn Stop Bomb","No Burn"};
#endif
