...
inline int canGoSingle(int i, int j, int curUtility) {
  ...
  if (grid[i][j] & HAS_BOMB_MASK)
    return 0;
  return (!(grid[i][j] & 0xc)) &&
         (utilities[i][j] > -1 * (BOMB_DEFAULT_TIMER - 530) ||
...
inline int canGo(int i, int j, int depth) {
  ...
  return (!(grid[i][j] & 0xc)) &&
         (utilities[i][j] >=
          -1 * ((BOMB_DEFAULT_TIMER - 530 - (1 + searchDepth - depth) * 530)));
}
...
int brainThink1(Brain* brain, Player* player) {
  ...
  if (brain->hasGoal) {
    ...
    if (brain->path && brain->path->head) {
      ...
      if (/*utilities[pi][pj]>1&&*/ brain->goal == GOAL_BOMB) {
        ...
        if (player->nBombsInUse != player->nBombs && canHide(pi, pj, 0) != -1) {
          ...
          brain->path = doBFS((player->y - YOFFSET + 16) >> 5,
                              (player->x - XOFFSET + 16) >> 5,
                              player->clientNum + 200, 6);
          // brain->hasGoal=1;
          // brain->actionToTake=0;
        }
      } else {
    ...
    q = doBFS((player->y - YOFFSET + 16) >> 5, (player->x - XOFFSET + 16) >> 5,
              player->clientNum + 200, 6);
    ...
    if (abs(GetTickCount() - brain->lastSearched) > 5) {
      q = doBFS((player->y - YOFFSET + 16) >> 5,
                (player->x - XOFFSET + 16) >> 5, 2, 8);
  ...
  while (i < GRID_HEIGHT) {
    ...
    while (j < GRID_WIDTH) {
      ...
      drawFont(surface, &font, str, (j << 5) + XOFFSET + 16,
               (i << 5) + YOFFSET + 16);
