...
int playKickSound(){ FSOUND_PlaySound(SOUND_CHANNEL,kickSound);return 1;}
int playPlayerSound(int p) {return 1;}
int playMenuItemSound(){FSOUND_PlaySound(SOUND_CHANNEL,menuItemSound);return 1;}
int playMenuSelectSound(){FSOUND_PlaySound(SOUND_CHANNEL,menuSelectedSound);return 1;}
int playBombSound(){FSOUND_PlaySound(SOUND_CHANNEL,bombSound);return 1;}
...
signed char endCallBack(FSOUND_STREAM * stream,void * buffer, int len, int param)
{
	if(mainStream!=NULL)
	{
		if(FSOUND_Stream_GetPosition(mainStream)>=FSOUND_Stream_GetLength(mainStream))
		nextSoundSyz();
	}
...
void stopSoundSyz()
{
	FSOUND_Stream_Stop(mainStream);
}
void setMusicVolume()
{
	FSOUND_SetVolume(MUSIC_CHANNEL,musicVolume);
}
...
void (* actions[10])(void)={backSoundSyz,playSoundSyz,pauseSoundSyz,
					stopSoundSyz,nextSoundSyz,addSoundSyz,removeSoundSyz,clearPlaylist,closeSoundSyz,openSoundSyz};
...
int loadSounds()
{
	bombSound = FSOUND_Sample_Load(FSOUND_FREE,"./data/sounds/Exp1.wav",FSOUND_NORMAL,0);
	kickSound = FSOUND_Sample_Load(FSOUND_FREE,"./data/sounds/kick.wav",FSOUND_NORMAL,0);
	menuItemSound = FSOUND_Sample_Load(FSOUND_FREE,"./data/sounds/menuitem.wav",FSOUND_NORMAL,0);
	menuSelectedSound = FSOUND_Sample_Load(FSOUND_FREE,"./data/sounds/menuselect.wav",FSOUND_NORMAL,0);
...
int initSoundSyz()
{
	...
	buttons[SCROLL_BOTTOM][0]=DDraw_Create_Surface(16,16,DDSCAPS_VIDEOMEMORY,0);
	buttons[SCROLL_BOTTOM][1]=DDraw_Create_Surface(16,16,DDSCAPS_VIDEOMEMORY,0);	
	buttons[SCROLL_MIDDLE][0]=DDraw_Create_Surface(16,16,DDSCAPS_VIDEOMEMORY,0);
	buttons[SCROLL_MIDDLE][1]=NULL;
	buttons[SCROLL_SLIDER][0]=DDraw_Create_Surface(16,16,DDSCAPS_VIDEOMEMORY,0);
...
inline int drawSoundSyzSHIDDEN()
{
	...
	DDraw_Draw_Surface(buttons[OPEN][selectedButton==OPEN],OPENX,sound_y+16,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[CLOSE][selectedButton==CLOSE],OPENX,sound_y+32,16,16,lpddsback,1);
...
int drawPlaylist(int y)
{
	...
	y=yback;
	DDraw_Draw_Surface(buttons[SCROLL_TOP][selectedButton==SCROLL_TOP],x,y,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[SCROLL_MIDDLE][0],x,y+16,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[SCROLL_MIDDLE][0],x,y+32,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[SCROLL_MIDDLE][0],x,y+48,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[SCROLL_MIDDLE][0],x,y+64,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[SCROLL_BOTTOM][selectedButton==SCROLL_BOTTOM],x,y+80,16,16,lpddsback,1);
	int slidery=y+8;
	if(numPlaylistEntries!=0)
		slidery+=(topPlaylist*72/numPlaylistEntries);
	DDraw_Draw_Surface(buttons[SCROLL_SLIDER][0],x,slidery,16,16,lpddsback,1);
...
int drawSoundSyzSOPEN()
{
	...
	if(playlistEntryNames[currentFile]!=NULL)
	{
		...
		if(offset>strlen(playlistEntryNames[currentFile]))
			offset=0;
		sprintf(title,"%s (%2d:%02d) %s",playlistEntryNames[currentFile]+((int)offset),min,sec,playlistEntryNames[currentFile]);
		drawFontBound(soundFont,title,x,sound_y-10,96,sound_y-10);
		offset+=0.1f;
	}
	while(i<5)
	{
		DDraw_Draw_Surface(buttons[i][selectedButton==i],x,y,16,16,lpddsback,1);
		i++;
		x+=16;
	}
	...
	DDraw_Draw_Surface(volume[VOL_SLIDER],(musicVolume<<6)/255+sound_x,y,16,16,lpddsback,1);
	i=5;
	x=sound_x;
	y+=16;
	while(i<8)
	...
	DDraw_Draw_Surface(buttons[OPEN][selectedButton==OPEN],sound_x+80,sound_y+16,16,16,lpddsback,1);
	DDraw_Draw_Surface(buttons[CLOSE][selectedButton==CLOSE],sound_x+80,sound_y+32,16,16,lpddsback,1);
...
inline int sendMessageSoundSyzSADDING(int message, int wparam, int lparam)
{
	...
	case WM_CHAR:
		...
		else addBuffer[addBufferLen++]=wparam;
