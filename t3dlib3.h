...
#define DM_NUM_SEGMENTS                                                        \
  64 // number of midi segments that can be cached in memory
...
// directsound
int DSound_Load_WAV(char* filename, int control_flags = (DSBCAPS_CTRLPAN |
                                                         DSBCAPS_CTRLFREQUENCY |
                                                         DSBCAPS_CTRLVOLUME));
int DSound_Replicate_Sound(int source_id);
int DSound_Play(int id, int flags = 0, int volume = 0, int rate = 0,
                int pan = 0);
