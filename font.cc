...
int bumpUp(Message* message) {
  ...
  while (finger != NULL) {
    ...
    if (inList != message && inList->x == message->x &&
        inList->y == message->y) {
    ...
    if (m->x == CHAT_X) {
      ...
      case 3:
        while (xx < m->x + getLength(m->font, m->message)) {
          ...
          SDL_BlitSurface(shadeTextures[shadeType], &src_rect, surface,
                          &dest_rect);
          xx += 16;
        }
        break;
...
/*
        ...
        post:
*/
int drawFont(SDL_Surface* surface, Font* font, const char* str, int* sx,
             int* sy) {
  int x = *sx;
  int y = *sy;
  int i = 0;
...
/*
        ...
        Post:
*/
int drawFontBound(SDL_Surface* surface, Font* font, const char* str, int sx,
...
int loadFont(Font* font, const char* str, int point) {
  ...
  while (i < 96) {
    ...
    font->letter[i] =
        SDL_CreateRGBSurface(0, font->width[i], point, 32, 0, 0, 0, 0);
